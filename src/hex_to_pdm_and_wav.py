...
def pdm_to_pcm(
    ...
    # Filtro FIR
    fir_filter = firwin(
        fir_size, cutoff_hz / (sample_rate / 2), window='hamming'
    )
    filtered_signal = lfilter(fir_filter, 1.0, pdm_bits)

    # Filtro Butterworth
    sos = butter(
        butter_order, cutoff_hz / (sample_rate / 2), btype='low', output='sos'
    )
    ...
    for sample in filtered_signal:
        output = rls_filter.predict(
            np.array([sample])
        )  # Passe como uma lista de um único elemento
    ...
    # Normalização
    pcm_signal = (pcm_signal - np.min(pcm_signal)) / (
        np.max(pcm_signal) - np.min(pcm_signal)
    )
...
def plot_pdm(pdm_bits, window_size):
    ...
    (l,) = plt.plot(
        pdm_bits[:window_size], linestyle='-', marker='', color='blue'
    )
...
def main():
    parser = argparse.ArgumentParser(
        description='Conversor de PDM para PCM com filtros configuráveis'
    )
    parser.add_argument(
        'filename', type=str, help='Arquivo .hex com dados PDM'
    )
    parser.add_argument(
        '--sample_rate',
        type=int,
        default=3072000,
        help='Taxa de amostragem do sinal PDM (default: 3072000 Hz)',
    )
    parser.add_argument(
        '--target_sample_rate',
        type=int,
        default=48000,
        help='Taxa de amostragem alvo para áudio PCM (default: 48000 Hz)',
    )
    parser.add_argument(
        '--fir_size',
        type=int,
        default=501,
        help='Tamanho do filtro FIR (default: 501)',
    )
    parser.add_argument(
        '--cutoff_hz',
        type=float,
        default=15000,
        help='Frequência de corte do filtro (default: 15000 Hz)',
    )
    parser.add_argument(
        '--butter_order',
        type=int,
        default=5,
        help='Ordem do filtro Butterworth (default: 5)',
    )
    parser.add_argument(
        '--notch_freq',
        type=float,
        default=0,
        help='Frequência central do filtro notch (0 para desativar)',
    )
    parser.add_argument(
        '--notch_q',
        type=float,
        default=30,
        help='Qualidade do filtro notch (default: 30)',
    )
    parser.add_argument(
        '--medfilt_size',
        type=int,
        default=1,
        help='Tamanho do filtro mediano (1 para desativar)',
    )
    parser.add_argument(
        '--output_file',
        type=str,
        default='output.wav',
        help='Nome do arquivo de saída (default: output.wav)',
    )
    parser.add_argument(
        '--window_size',
        type=int,
        default=1000,
        help='Tamanho da janela para visualização do sinal PDM (default: 1000)',
    )
